...
* `exporter_add_timeout` bounds how long the load balancer waits for a new backend exporter to be created and started when the resolver reports a backend change. A backend that is not ready in time is left out of the hash ring, so its share of the data is routed to the backends that are ready, and it joins the ring as soon as it finishes starting. While no backend is ready at all the wait continues past this timeout, because giving up would leave nothing to route to and every batch would be rejected. Set to `0` to wait indefinitely. If not specified, `5s` will be used.
* `exporter_shutdown_timeout` bounds how long the load balancer waits for a removed backend exporter to drain in-flight data and shut down when the resolver reports a backend change. When it expires the backend is shut down regardless, so set this to at least the export `timeout` you have configured, otherwise a backend leaving the resolved set can cut off an in-flight export. Set to `0` to wait indefinitely. If not specified, `30s` will be used.
